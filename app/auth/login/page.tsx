...
function LoginForm() {
  const { signInWithEmail, signUpWithEmail, resetPasswordForEmail, signInWithOAuth, user } = useAuth();
  const router = useRouter();
  const searchParams = useSearchParams();

  // Open-redirect protection: Only permit internal relative paths
  const rawRedirect = searchParams.get('redirect') || searchParams.get('returnUrl') || '/ai';
  const safeRedirect = rawRedirect.startsWith('/') && !rawRedirect.startsWith('//') ? rawRedirect : '/ai';
  ...
  // If already signed in, display confirmation and continue button
  if (user) {
    return (
      <div className="text-center p-8 rounded-3xl bg-[#0F1738] border border-indigo-500/40 text-slate-100 max-w-md w-full shadow-2xl">
        ...
        <Link
          href={safeRedirect}
          className="inline-flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 font-bold text-xs text-white shadow-lg shadow-indigo-600/30 hover:scale-[1.02] transition-all"
        >
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'signin') {
        ...
        const { error, user: newUser } = await signUpWithEmail(email.trim(), password);
        if (error) {
          ...
          if (newUser?.identities?.length === 0) {
            ...
            setSuccessMsg(
              'Account created! Check your email if verification is required, or sign in now.'
            );
            setTimeout(() => {
              setMode('signin');
            }, 2500);
      ...
      >
        ...
        <div className="flex flex-col items-center text-center mb-6 relative z-10">
          ...
          <p className="text-xs text-slate-400 mt-1 max-w-sm">
            Access multi-model AI reasoning (Claude 3.5, Gemini 3.6, Groq), grounded PDF analysis, and live video tutoring.
          </p>
        </div>
          ...
          <>
            ...
            <div className="relative z-10">
              <button
                ...
                disabled={loading}
                className="w-full py-2.5 px-3 rounded-xl bg-slate-900/80 hover:bg-slate-800 border border-slate-800 hover:border-slate-700 text-xs font-medium text-slate-200 transition-all flex items-center justify-center gap-2"
              >
